...
class TestUIManager:
    ...
    def test_get_shadow(
        ...
        shadow_surface = default_ui_manager.get_shadow(
            size=requested_size,
            shadow_width=2,
            shape="rectangle",
        ...
        assert isinstance(shadow_surface, pygame.Surface) and (
            shadow_surface_size == requested_size
        )

    def test_set_window_resolution(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        """
    ...
    def test_clear_and_reset(
        ...
        should_be_one_sprite_again = len(
            default_ui_manager.get_sprite_group().sprites()
        )

        assert (
            should_be_one_sprite == 1
            and should_be_two_sprites == 2
            and should_be_one_sprite_again == 1
        )
    ...
    def test_process_events(
        ...
        UIMessageWindow(
            rect=pygame.Rect(500, 100, 250, 300),
        ...
        default_ui_manager.process_events(
            pygame.event.Event(pygame.MOUSEBUTTONDOWN, {"button": 1, "pos": (125, 115)})
        )
        assert test_button.held

    def test_update(
        self, _init_pygame, default_ui_manager: UIManager, _display_surface_return_none
    ):
        """
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["xy"]
        )
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["yx"]
        )
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["xl"]
        )
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["xr"]
        )
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["yt"]
        )
        ...
        assert (
            default_ui_manager._active_cursor
            == default_ui_manager.resizing_window_cursors["yb"]
        )
        ...
        default_ui_manager.update(0.05)
        assert (
            default_ui_manager._active_cursor == default_ui_manager.active_user_cursor
        )
    ...
    def test_draw_ui(self, _init_pygame, _display_surface_return_none):
        ...
        if plat == "WINDOWS":
            comparison_surface = pygame.image.load(
                os.path.join(
                    "tests", "comparison_images", "test_ui_manager_draw_ui.png"
                )
            ).convert_alpha()
        else:
            comparison_surface = pygame.image.load(
                os.path.join(
                    "tests", "comparison_images", "test_ui_manager_draw_ui_linux.png"
                )
            ).convert_alpha()
        ...
        result_pixel_array = test_pixel_array.compare(
            comparison_pixel_array, distance=0.05
        )
        ...
        for x in range(0, 150):
            for y in range(0, 30):
                assert (
                    result_surface.unmap_rgb(result_pixel_array[x, y])
                    == no_mismatch_colour
                ), f"Colours not equal at: {x}, {y}"
        try:
    ...
    def test_add_font_paths_and_preload_fonts(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        """
        ...
        default_ui_manager.preload_fonts(
            [{"name": "roboto", "point_size": 14, "style": "regular"}]
        )
        default_ui_manager.preload_fonts(
            [{"name": "noto_sans", "html_size": 3, "style": "italic"}]
        )
        # default_ui_manager.resource_loader.start()
        # default_ui_manager.resource_loader.update()

        UITextBox(
            html_text="<font face=roboto>Test font pre-loading</font>",
            relative_rect=pygame.Rect(100, 100, 200, 100),
            manager=default_ui_manager,
        )

    def test_print_unused_fonts(
        self, _init_pygame, default_ui_manager, _display_surface_return_none, capsys
    ):
        """
    ...
    def test_focus_and_unfocus_focus_element(
        self, _init_pygame, default_ui_manager, _display_surface_return_none
    ):
        """
    ...
    def test_incremental_loading_nothing(
    ...
    def test_hover_of_hidden_elements(self, _init_pygame, _display_surface_return_none):
        ...
        button1 = UIButton(
            ...
            text="Lower button test",
            manager=manager,
            starting_height=1,
        )
        button2 = UIButton(
            relative_rect=pygame.Rect(100, 100, 100, 100),
            text="Higher button test",
            manager=manager,
            starting_height=2,
        )

        # override of default manager _update_mouse_position method to simulate hovering over the button
        manager._update_mouse_position = update_mouse_position_override_factory(
            manager, 150, 150
        )
    ...
    def test_set_visual_debug(self, _init_pygame, _display_surface_return_none):
        ...
        UIButton(
            relative_rect=pygame.Rect(100, 100, 150, 30), text="Test", manager=manager
        )
    ...
    def test_set_active_cursor(self, _init_pygame, _display_surface_return_none):
        ...
        assert manager._active_cursor == pygame.cursors.Cursor(
    ...
    def test_translation_dir_path(self):
        UIManager(
            (800, 600), translation_directory_paths=["pygame_gui/data/translations/"]
        )

    def test_incremental_loading_something(
        self, _init_pygame, _display_surface_return_none
    ):
        incremental_loader = IncrementalThreadedResourceLoader()
        ...
        UIManager((800, 600), theme_package, resource_loader=incremental_loader)
